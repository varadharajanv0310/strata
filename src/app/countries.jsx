    ...
    return (
      <div className="wrap-wide surface-enter">
        <div className="sec-head" style={{ marginBottom: 24 }}>
          <div><div className="sec-eyebrow">Countries</div><div className="h1" style={{ color: "var(--white)" }}>Markets, end to end</div>
        ...
        <div className="card country-strip-card" style={{ marginBottom: 16, overflowX: "auto" }}>
          ...
          <div className="row gap10 country-strip" style={{ minWidth: 700 }}>
            {S().COUNTRIES.map(co => {
              ...
              return (
                <button key={co.code} onClick={() => setCode(co.code)} className="col gap10" style={{ flex: 1, background: code === co.code ? "rgba(111,162,255,0.12)" : "transparent", border: "1px solid " + (code === co.code ? "rgba(111,162,255,0.5)" : "var(--line)"), borderRadius: 12, padding: "14px 12px", cursor: "pointer", alignItems: "stretch" }}>
                  ...
                  <div className="tnum" style={{ fontSize: 18, fontWeight: 700, color: "var(--white)" }}>◊{Math.round(s.avgPPP / 1000)}k</div>
                  <div style={{ height: 4, borderRadius: 9, background: "var(--wash-2)", overflow: "hidden" }}><div style={{ width: `${(s.avgPPP / mx) * 100}%`, height: "100%", background: "var(--bar)" }} /></div>
                </button>
              );
            })}
          </div>
        ...
        <div className="grid" style={{ gridTemplateColumns: "1.4fr 1fr 1fr 1.2fr", gap: 16, marginBottom: 16 }}>
          <div className="card pooled" style={{ justifyContent: "space-between" }}>
            <div className="row gap12" style={{ alignItems: "center" }}><ui.CountryDot code={code} size={20} /><div className="h2" style={{ color: "var(--white)" }}>{c.name}</div></div>
            <div className="row between mt20"><div><div className="stat-label">Currency</div><div style={{ fontSize: 18, fontWeight: 700, color: "var(--white)", marginTop: 4 }}>{c.cur} {c.curCode}</div></div>
              <button className="pill sm" onClick={() => app.go("compare", { mode: "mirror" })}>Mirror →</button></div>
          </div>
          <div className="card"><span className="stat-label">Avg pay (PPP)</span><div className="tnum" style={{ fontSize: 30, fontWeight: 700, color: "var(--white)", marginTop: 12 }}>◊{Math.round(st.avgPPP / 1000)}k</div><div className="small mt8" style={{ color: "var(--t3)" }}>across 16 roles</div></div>
              ...
              {byDisclosure.slice(0, 5).map(r => {
                ...
                return (
                  <div key={r.id} className="row gap12" style={{ alignItems: "center" }}>
                    ...
                    <div style={{ flex: 1, height: 8, background: "var(--wash)", borderRadius: 9, overflow: "hidden" }}>
                      <div style={{ width: `${t * 100}%`, height: "100%", borderRadius: 9, background: t >= 0.6 ? "var(--good)" : t >= 0.4 ? "var(--warn)" : "var(--bad)" }} />
                    </div>
                    <span className="tnum" style={{ width: 40, textAlign: "right", fontSize: 13, fontWeight: 700, color: "var(--white)" }}>{Math.round(t * 100)}%</span>
                  </div>
                );
              })}
            </div>
