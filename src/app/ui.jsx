...
/* ============================================================
  ...
  function InteractiveGlobe({ size = 440, active, onSelect }) {
    ...
    useEffect(() => {
      ...
      const draw = (advance) => {
        ...
        g.addColorStop(0, "#243049"); g.addColorStop(0.5, "#121a29"); g.addColorStop(1, "#05070c");
        ...
        for (let i = 0; i < land.length; i++) {
          ...
          const a = 0.2 + bright * 0.62;
          const cr = Math.round(150 + bright * 105), cg = Math.round(170 + bright * 85), cb = Math.round(214 + bright * 41);
          const rad = (0.85 + bright * 1.15) * (0.55 + 0.45 * z);
          ctx.beginPath(); ctx.arc(cx + R * x, cy - R * y, rad, 0, 7); ctx.fillStyle = "rgba(" + cr + "," + cg + "," + cb + "," + a + ")"; ctx.fill();
        }
        ctx.beginPath(); ctx.arc(cx, cy, R, 0, 7); ctx.strokeStyle = "rgba(160,185,235,0.28)"; ctx.lineWidth = 1; ctx.stroke();
        const nodes = [];
        for (const co of S().COUNTRIES) {
          ...
          if (isA) { const pr = 11 + Math.sin(time * 2.4) * 4; ctx.beginPath(); ctx.arc(sx, sy, pr, 0, 7); ctx.strokeStyle = "rgba(111,162,255,0.7)"; ctx.lineWidth = 1.5; ctx.stroke(); }
          ctx.beginPath(); ctx.arc(sx, sy, (isA ? 8 : 5.5), 0, 7); ctx.fillStyle = "rgba(111,162,255,0.22)"; ctx.fill();
          ctx.beginPath(); ctx.arc(sx, sy, (isA ? 5 : isH ? 4.5 : 3.8), 0, 7); ctx.fillStyle = isA ? "#9CC4FF" : "#dbe4f7"; ctx.fill();
          ctx.lineWidth = isA ? 2.4 : 1.4; ctx.strokeStyle = "#0a1422"; ctx.stroke();
        }
        ...
        for (const n of nodes) {
          ...
          ctx.fillStyle = "rgba(12,16,26,0.94)"; rr(ctx, lx, n.y - 12, w, 22, 11); ctx.fill();
          ctx.strokeStyle = "rgba(160,185,235,0.4)"; ctx.lineWidth = 1; rr(ctx, lx, n.y - 12, w, 22, 11); ctx.stroke();
          ctx.fillStyle = "#F4F6FB"; ctx.textBaseline = "middle"; ctx.textAlign = leftSide ? "right" : "left";
      ...
      <div ref={ref} style={{ position: "relative" }}>
        ...
        {open && (
          <div style={{
            ...
            padding: 6, boxShadow: "0 24px 64px rgba(0,0,0,0.6)", animation: "fadeUp 0.16s ease"
          }}>
            {S().COUNTRIES.map(co => (
              <button key={co.code} onClick={() => { onChange(co.code); setOpen(false); }}
                className="row gap10" style={{
                  ...
                  background: co.code === value ? "rgba(111,162,255,0.16)" : "transparent",
  ...
  function BigSalary({ value, code, size = 44 }) {
    ...
    return <span className="tnum" style={{ fontSize: size, fontWeight: 700, letterSpacing: "-0.03em", color: "var(--white)", lineHeight: 1 }}>
        ...
        {comps.map(c => (
          <div key={c.k} className="comp">
            <div className="row between"><span className="small" style={{ color: "var(--t2)" }}>{c.k}</span>
              <span className="tnum" style={{ fontSize: 13, fontWeight: 700, color: "var(--white)" }}>{c.v.toFixed(1)}</span></div>
  ...
  function FamilyDot({ family, size = 8 }) {
    return <span className="fam-dot" style={{ width: size, height: size, borderRadius: 999, display: "inline-block",
      background: `oklch(0.72 0.17 ${family.hue})` }} />;
  }
