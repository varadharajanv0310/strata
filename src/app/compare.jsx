...
/* ============================================================
  ...
  function RolePickerInline({ onPick, exclude = [], label = "+ Add role" }) {
    ...
    return (
      <div ref={ref} style={{ position: "relative" }}>
        ...
        {open && (
          <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, zIndex: 80, width: 250, maxHeight: 300, overflowY: "auto", background: "var(--ink-2)", border: "1px solid var(--glass-line)", borderRadius: 14, padding: 6, boxShadow: "0 24px 64px rgba(0,0,0,0.6)", }}>
            {avail.map(r => (
                ...
                {metrics.map(m => {
                  ...
                  return (
                    <tr key={m.k} style={{ cursor: "default" }}>
                      ...
                      {roles.map((r, i) => (
                        <td key={r.id} className="num" style={{ fontWeight: raws && raws[i] === best ? 700 : 500, color: raws && raws[i] === best ? "var(--white)" : "var(--t2)", background: raws && raws[i] === best ? "rgba(111,162,255,0.12)" : "transparent" }}>
          ...
          <div className="col" style={{ gap: 11 }}>
            {items.map(it => (
              <div key={it.code} className="row gap12" style={{ alignItems: "center" }}>
                ...
                <span className="tnum" style={{ width: 130, textAlign: "right", fontSize: 14, fontWeight: 700, color: "var(--white)" }}>
  ...
  function YearPicker({ value, onChange }) {
    return <select className="pill" style={{ appearance: "none", paddingRight: 28 }} value={value} onChange={e => onChange(+e.target.value)}>
      {S().YEARS.map(y => <option key={y} value={y} style={{ background: "var(--ink-2)", color: "var(--white)" }}>{y}</option>)}
    </select>;
  }
    ...
    return (
      <div className="card pooled">
        ...
        <div className="row gap16 mt16" style={{ alignItems: "baseline", flexWrap: "wrap" }}>
          ...
          <div><div className="small" style={{ color: "var(--t3)" }}>now</div><div className="tnum" style={{ fontSize: 30, fontWeight: 700, color: "var(--white)" }}>{now}</div></div>
    ...
    return (
      <div>
        ...
        <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          {[[a, sa], [b, sb]].map(([code, s]) => (
            <div key={code} className="card pooled">
              <div className="row gap10" style={{ marginBottom: 18, alignItems: "center" }}><Uc.CountryDot code={code} size={16} /><span className="h3" style={{ color: "var(--white)" }}>{S().C[code].name}</span></div>
              <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 18 }}>
                <div><div className="stat-label">Avg pay (PPP)</div><div className="tnum" style={{ fontSize: 24, fontWeight: 700, color: "var(--white)", marginTop: 4 }}>◊{Math.round(s.avgPPP / 1000)}k</div></div>
              ...
              <div className="col" style={{ gap: 8 }}>
                {s.top.map((r, i) => (
                  <div key={r.id} className="row between" style={{ fontSize: 13 }}>
                    ...
                    <span className="tnum" style={{ fontWeight: 700, color: "var(--white)" }}>{S().fmtCompact(r.countries[code].median, code)}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="card pooled" style={{ marginTop: 16 }}>
          ...
          <Charts.MultiLine height={220}
            series={[[a, "#6FA2FF"], [b, "#E8B765"]].map(([code, color]) => ({
          ...
          <div className="row gap16 wrap-f mt12">
            <span className="row gap6" style={{ alignItems: "center", fontSize: 12, color: "var(--t2)" }}><span style={{ width: 14, height: 3, background: "#6FA2FF", borderRadius: 2, display: "inline-block" }}></span>{S().C[a].name}</span>
            <span className="row gap6" style={{ alignItems: "center", fontSize: 12, color: "var(--t2)" }}><span style={{ width: 14, height: 3, background: "#E8B765", borderRadius: 2, display: "inline-block" }}></span>{S().C[b].name}</span>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="wrap-wide surface-enter">
        <div className="sec-head" style={{ marginBottom: 22 }}>
          <div><div className="sec-eyebrow">Compare</div><div className="h1" style={{ color: "var(--white)" }}>Hold anything side by side</div>
