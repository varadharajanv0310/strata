...
/* ============================================================
            ...
            {items.map((r, i) => {
              ...
              return (
                <button key={r.id} onClick={(e) => window.openRoleMenu(r.id)} className="row between" style={{ width: "100%", background: "transparent", border: "none", cursor: "pointer", padding: "11px 0", borderBottom: i < items.length - 1 ? "1px solid var(--line)" : "none", textAlign: "left" }}>
                  ...
                  <span className="tnum" style={{ fontSize: 13.5, fontWeight: 700, color: "#fff", flexShrink: 0 }}>{fmt(getv(cd))}</span>
                </button>
              );
            })}
          </div>
  ...
  function MobileScoreRow({ role, code, app, n }) {
    ...
    return (
      <div style={{ borderRadius: 14, background: open ? "rgba(42,91,255,0.06)" : "transparent", border: "1px solid " + (open ? "rgba(74,124,255,0.28)" : "transparent"), marginBottom: 2, transition: "background 0.2s" }}>
            ...
            <div className="col gap10" style={{ padding: "10px 0", borderTop: "1px solid var(--line)" }}>
              {comps.map(([k, v]) => (
                <div key={k} className="row gap10" style={{ alignItems: "center" }}>
                  ...
                  <div style={{ flex: 1, height: 6, borderRadius: 9, background: "rgba(255,255,255,0.07)", overflow: "hidden" }}>
                    <div style={{ width: `${v * 10}%`, height: "100%", borderRadius: 9, background: "linear-gradient(90deg,#0033ff,#4a7cff)" }} />
                  </div>
                  <span className="tnum" style={{ width: 28, textAlign: "right", fontSize: 12.5, fontWeight: 700, color: "#fff" }}>{v.toFixed(1)}</span>
                </div>
              ))}
            </div>
      ...
      <div className="surface-enter">
        ...
        {q.trim() === "" ? (
          ...
          <div className="row between" style={{ marginBottom: 14 }}>
            <div className="small" style={{ color: "var(--t2)" }}><span style={{ color: "#fff", fontWeight: 700 }}>{filtered.length}</span> matching "<span style={{ color: "var(--sky)" }}>{q}</span>"</div>
          ...
          {filtered.map(r => {
            ...
            return (
              <button key={r.id} onClick={() => app.go("roles", { roleId: r.id })} className="card pooled lift" style={{ cursor: "pointer", textAlign: "left", border: "1px solid var(--glass-line)", display: "block", width: "100%" }}>
                ...
                <div className="h3" style={{ color: "#fff", fontSize: 16 }}>{r.name}</div>
